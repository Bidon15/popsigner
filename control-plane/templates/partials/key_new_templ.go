...
func KeyNewModal(namespaces []*models.Namespace) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 8, "</select><p class=\"mt-1.5 text-xs text-bao-muted\">Keys are organized by namespace (e.g., production, staging)</p></div><!-- Algorithm (read-only for now) --><div><label class=\"block text-sm font-medium text-bao-text mb-2\">Algorithm</label><div class=\"flex items-center gap-3 px-4 py-2.5 bg-bao-border/30 border border-bao-border rounded-xl\"><span class=\"text-bao-text\">secp256k1</span> <span class=\"text-xs text-bao-muted bg-bao-border/50 px-2 py-0.5 rounded\">Ethereum / Bitcoin compatible</span></div><input type=\"hidden\" name=\"algorithm\" value=\"secp256k1\"></div><!-- Network Type Selection --><div><label class=\"block text-sm font-medium text-bao-text mb-2\">Primary Network</label> <select name=\"network_type\" required class=\"w-full px-4 py-2.5 bg-bao-bg border border-bao-border rounded-xl text-bao-text focus:border-bao-accent focus:outline-none focus:ring-1 focus:ring-bao-accent/50 cursor-pointer transition-colors\"><option value=\"evm\">⟠ EVM (OP Stack, Ethereum)</option> <option value=\"celestia\">🌌 Celestia (DA Layer)</option> <option value=\"all\" selected>🔗 Universal (Both Networks)</option></select><p class=\"mt-1.5 text-xs text-bao-muted\">Same key works on both networks. This sets your default address display.</p></div><!-- Exportable Option --><div class=\"p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl\"><label class=\"flex items-start gap-3 cursor-pointer\"><input type=\"checkbox\" name=\"exportable\" value=\"true\" class=\"mt-0.5 w-4 h-4 rounded border-bao-border bg-bao-bg text-bao-accent focus:ring-bao-accent focus:ring-offset-0 cursor-pointer\"><div><span class=\"text-sm font-medium text-bao-text flex items-center gap-2\">Allow export <span class=\"text-xs text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded\">⚠️ Less secure</span></span><p class=\"mt-1 text-xs text-bao-muted\">Enables exporting the private key. Only enable if you need to migrate keys elsewhere.</p></div></label></div></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
