...
import (
	...
	"github.com/Bidon15/popsigner/control-plane/templates/partials"
)
...
func (h *WebHandler) KeysDelete(w http.ResponseWriter, r *http.Request) {
	...
	// Otherwise return just the list partial
	templ.Handler(pages.KeysList(keys, namespaces)).ServeHTTP(w, r)
}
...
// If the user has no organization, it creates a default one.
func (h *WebHandler) getUserAndOrg(r *http.Request) (*models.User, *models.Organization, error) {
	...
	// If still no org, create a default one for the user
	if org == nil {
		defaultOrgName := "My Organization"
		if user.Name != nil && *user.Name != "" {
			defaultOrgName = *user.Name + "'s Organization"
		}
		org, err = h.orgService.Create(r.Context(), defaultOrgName, user.ID)
		if err != nil {
