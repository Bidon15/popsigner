...
func GenerateEnvExample(cfg *DeploymentConfig, addrs *ContractAddresses) string {
	...
	// Use generated namespace or default
...
# 
# This file contains all environment variables needed to run your OP Stack.
...
# Chain name (for logging and identification)
CHAIN_NAME=%s

################################################################################
# POPSIGNER - Secure Key Management
################################################################################

# POPSigner RPC endpoint for transaction signing
POPSIGNER_ENDPOINT=%s

# Your POPSigner API key (get from https://popsigner.com/dashboard)
POPSIGNER_API_KEY=<REQUIRED: your_api_key>

################################################################################
# ROLE ADDRESSES - These are your POPSigner-managed keys
################################################################################

# Sequencer: Signs L2 blocks (unsafe head)
SEQUENCER_ADDRESS=%s
...
# L2OutputOracle proxy address (set after deployment)
L2_OUTPUT_ORACLE_ADDRESS=%s

################################################################################
# CELESTIA DATA AVAILABILITY - Required for op-alt-da
################################################################################
# POPKins uses Celestia for data availability. Configure these values
# based on your Celestia node setup.

# Your chain's Celestia namespace (10 bytes, hex-encoded)
# This was auto-generated based on your chain ID
CELESTIA_NAMESPACE=%s

# Celestia consensus node gRPC endpoint
# Testnet (mocha-4): consensus-full-mocha-4.celestia-mocha.com:9090
# Mainnet: Use your own node or public endpoint
CELESTIA_CORE_GRPC=%s

# Celestia keyring configuration
# Create key with: celestia-appd keys add <key_name>
CELESTIA_KEY_NAME=<REQUIRED: your_celestia_key_name>
...
# Gas limit for L2 blocks (default: 30000000)
# GAS_LIMIT=30000000

# L2 block time in seconds (default: 2)
# BLOCK_TIME=2
`,
		cfg.ChainName,
		cfg.L1RPC,
