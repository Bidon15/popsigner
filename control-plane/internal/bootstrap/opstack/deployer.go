...
func (d *OPDeployer) Deploy(ctx context.Context, cfg *DeploymentConfig, signerAdapter *POPSignerAdapter) (*DeployResult, error) {
	...
	// 4. Download or extract artifacts
	d.logger.Info("downloading contract artifacts")
	l1Artifacts, err := artifacts.Download(ctx, artifacts.DefaultL1ContractsLocator, nil, d.cacheDir)
	if err != nil {
		return nil, fmt.Errorf("download L1 artifacts: %w", err)
	}
	l2Artifacts, err := artifacts.Download(ctx, artifacts.DefaultL2ContractsLocator, nil, d.cacheDir)
	if err != nil {
